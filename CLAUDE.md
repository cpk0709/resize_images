...
scripts/package-standalone.mjs  next build(standalone) 결과에 static·public 을 넣고 .env* 를 제거
Dockerfile, .dockerignore  운영 이미지 (deps → build → runtime, 비루트). GitHub Actions 가 ghcr.io/cpk0709/docufit 로 push
deploy/docker/            **현재 운영 방식.** 공유 Docker 호스트용: compose.yml(127.0.0.1:3100), setup-shared.sh(기존 nginx 에 사이트만 추가), release.sh(pull→up→헬스→롤백→이미지 정리)
deploy/ec2/               전용 인스턴스 변형(systemd + 호스트 Node). 현재 워크플로는 쓰지 않음
deploy/nginx/             docufit.conf(도메인, certbot 이 443 추가) · docufit-port.conf(도메인 전 임시 8080)
.github/workflows/deploy-ec2.yml  main push → 이미지 빌드·GHCR push → scp compose/release.sh → release.sh (변수 EC2_DEPLOY_ENABLED=true 일 때만)
...
- **배포는 EC2 공유 Docker 호스트 + 호스트 nginx** (세션 21, 인스턴스 `medibuilder-platform` 3.34.61.39). 기존 앱 2개가 127.0.0.1:3000/3001 컨테이너, 호스트 nginx 가 80/443 이름 기반 가상호스트. DocuFit 은 **컨테이너로 127.0.0.1:3100**, nginx 사이트 `docufit` 하나만 추가. **서버의 기존 서비스·nginx.conf·ufw 를 건드리는 스크립트를 쓰지 않는다** (setup-shared.sh 는 사이트 파일 추가와 8080 임시 규칙만). 디스크가 빠듯(≈6GB)하니 이미지는 현재·이전 태그만 남긴다(release.sh). 서버 환경변수의 유일한 출처는 `/srv/docufit/app.env`; 이미지에 `.env` 가 실리지 않게 `package-standalone.mjs` 가 지운다. 이미지는 glibc(bookworm-slim) x86_64 — 빌드 러너와 같은 아키텍처. `headers()` 의 no-store 는 `/_next/static` 을 제외한다 — 해시 자산까지 no-store 를 걸면 방문마다 1MB 넘는 JS 를 다시 받는다.
...
## UI 규칙 (사용자 피드백으로 확정, 세션 12~20)
- **디자인 기준은 `docs/design/studio-concept-v2.png`** (세션 20, 사용자가 "회사 디자인팀이 만든 것처럼" 요청해 받은 시안). v1(Gemini) 은 참고용으로만 남긴다. 한 가지 파랑(`brand` #2563eb)만 쓰고, 그라데이션·남색(navy)·에메랄드 hero 색은 폐기했다. "AI 가 만든 듯한" 인상을 주는 요소(과한 그라데이션, 큰 둥근 배지, 이모지·유니코드 기호 아이콘, 큼직한 제목)를 피한다.
- **버튼 색·크기는 `src/components/ui/Button.tsx` 에서만 정한다.** 다른 컴포넌트에서 버튼 색 클래스를 직접 쓰지 않는다. 아이콘은 `src/components/ui/icons.tsx` 의 단색 선 아이콘만 쓴다. **이모지·유니코드 기호(■ ▦ ↻ ⇅ ◀) 금지** (OS 마다 모양·색이 달라 톤을 깬다).
- **검은 채움·그라데이션 버튼 금지.** 선택 토글 = 연한 파랑 배경 + 파랑 테두리 + 파랑 글자(`bg-brand-soft border-brand text-brand`). primary = brand 단색 채움. secondary = 흰 배경 + 테두리. ghost = 배경 없음.
- **최종 추출 버튼(`hero`)은 화면에 하나만.** primary 와 같은 파랑이되 lg 크기 + 파랑 그림자로 구별한다. 위치는 작업 흐름의 끝인 **오른쪽 패널 하단** (요약 한 줄 + 패널 폭을 채우는 버튼). 다른 어떤 요소에도 재사용하지 않는다.
- **xl 이상은 앱 셸**: `main` 높이 = `100dvh − 앱 바 3.5rem − 푸터 2.5rem`, 세 패널이 안에서 스크롤(`scroll-thin xl:min-h-0 xl:overflow-y-auto`), 맨 아래 상태 바. 푸터 높이(h-10)나 앱 바 높이(h-14)를 바꾸면 `page.tsx` 의 계산도 바꾼다. 세로 flex 자식에 `flex-1` 이 남아 있으면 height 가 무시되므로 xl 에서 `flex-none`.
- **글꼴은 next/font (Inter + Noto Sans KR)** 로 빌드 시 내려받아 함께 배포한다. 실행 중 폰트 CDN 요청이 나가면 프라이버시 원칙 위반이다. 본문 13px, 패널 제목 15px semibold, 보조 11px, 숫자는 `tabular-nums`.
...
- **모바일(lg 미만) 배치:** 서류 추가(편집 캔버스) → 설정(컨트롤 패널) → 미리보기 순서(`order-*`), 최종 버튼은 화면 하단 고정 바(`ActionBar` 의 `lg:hidden` 쪽), `main` 에 `pb-28` 로 가림 방지. 프리셋은 가로 스크롤 칩.
- **모바일 grid 는 반드시 `grid-cols-1`, 패널은 `min-w-0`.** 열 정의 없는 grid 는 암시적 auto 열이 자식의 max-content(가로 스크롤 칩 등)까지 늘어나 레이아웃 뷰포트가 폰 폭을 넘고 페이지가 축소되어 보인다. viewport 메타가 있어도 생긴다 (세션 16).
- **아이콘 전용 버튼은 `IconButton`** (36px 정사각, `label` 필수 → aria-label/title). 탭 타깃을 그보다 줄이지 않는다.
- **에디터 확대는 fabric 뷰포트 줌(`setZoom` + `setDimensions`)** 으로만 한다. scene 좌표는 맞춤 배율 그대로라 영역 ↔ 이미지 픽셀 환산을 건드리지 않는다. 캔버스 위에 컨트롤을 띄우지 않는다(서류 모서리를 가림). fabric 캔버스는 `touch-action: none` 이라 확대 상태의 손가락 스크롤은 "이동 모드" 오버레이가 담당한다 (세션 19).
- **캔버스 컨테이너 안의 넘침은 `overflow-auto` + 내부 `m-auto` 래퍼**로 처리한다. `justify-center` 는 넘친 왼쪽·위를 잘라 스크롤로 닿을 수 없게 만든다.
...
- 모바일 검증은 CDP `Emulation.setDeviceMetricsOverride`(390×844, DPR 3, mobile) 로 하고 **`innerWidth === 390` 을 먼저 확인**한다. 이 값이 크면 어떤 요소가 레이아웃 뷰포트를 넓힌 것이다. `scrollWidth <= innerWidth` 만 보면 둘이 같이 커져 통과하는 착시가 난다 (세션 16).
