...
- **정식 배포(진행 중, 세션 21):** **EC2 공유 Docker 호스트** `medibuilder-platform`(i-0260fcd690f2a86b7, EIP 3.34.61.39, Ubuntu 24.04 x86_64, t3.medium, 기존 앱 2개 + 호스트 nginx). DocuFit 은 컨테이너(127.0.0.1:3100) + nginx 사이트 추가. 저장소 쪽 준비 완료 — `Dockerfile`, `deploy/docker/{compose.yml,setup-shared.sh,release.sh}`, `deploy/nginx/{docufit.conf,docufit-port.conf}`, `.github/workflows/deploy-ec2.yml`(GHCR push → SSH release; 변수 `EC2_DEPLOY_ENABLED=true` 일 때만). **사용자 쪽 남은 일:** 서버에서 `setup-shared.sh`(8080 임시 모드) → 배포 키·Secrets·Variables 등록 → push 로 첫 배포 → GHCR 패키지 Public → 보안 그룹 8080 → 확인 → 도메인 구매·A 레코드 → `setup-shared.sh --domain --email`. 절차는 README "배포 (EC2, 정식)".
...
2. **EC2 정식 배포 실행** (사용자 결정, 세션 21) — 대상: 운영 중인 공유 Docker 호스트 `medibuilder-platform`(3.34.61.39). 사용자: 서버에서 `git clone` → `sudo bash deploy/docker/setup-shared.sh`(8080 임시 모드) → 배포 키 `docufit-deploy` 생성·`authorized_keys` 등록 → Secrets(EC2_HOST/EC2_USER/EC2_SSH_KEY) + Variables(EC2_DEPLOY_ENABLED=true, DEPLOY_URL=http://3.34.61.39:8080/) → 보안 그룹 8080 → push 로 첫 배포 → GHCR 패키지 Public → 도메인 구매·A 레코드 → `setup-shared.sh --domain --email` + `DEPLOY_URL` 갱신. (전용 인스턴스용 `deploy/ec2/setup.sh` 변형은 보관만.) 그 뒤: `curl -sI https://<도메인>/ | grep -i cache-control` 로 `no-store` 확인, `src/app/privacy/page.tsx` 3절에 "AWS 서울 EC2 · nginx 접속 로그 보관 기간" 기입, GitHub Pages 워크플로 유지 여부 결정, `metadataBase` 를 새 도메인으로.
...
- **두 번째 후보 → 채택 (결정):** 사용자가 운영 중인 `medibuilder-platform`(i-0260fcd690f2a86b7, **EIP 3.34.61.39**, 기본 VPC 퍼블릭 서브넷, 공식 Ubuntu 24.04 amd64 AMI, t3.medium, 키 `new-medi`)에 "다른 포트로 함께 돌리자" 고 제안. SSH 감사 결과: RAM 3.7GB(가용 2.2GB), **디스크 78% 사용(6.3GB 남음)**, Docker 29 + compose v5, 기존 컨테이너 `medi-scheduler`(127.0.0.1:3000)·`lsn-survey`(127.0.0.1:3001), **호스트 nginx 가 80/443** 을 잡고 `sites-enabled/{lsh-sleep-survey,medi-scheduler}` 이름 기반 가상호스트, ufw active, 호스트 Node 없음, 재부팅 대기(커널 업데이트). → **적합.** 결정: (1) 호스트에 Node·nginx 를 새로 깔지 않고 **DocuFit 도 컨테이너**(127.0.0.1:3100)로, 기존 nginx 에 사이트 하나만 추가. (2) 이미지는 CI 가 빌드해 GHCR 로 (서버 RAM·디스크 보호). (3) ufw 는 건드리지 않음(8080 임시 규칙만). (4) 도메인 전까지는 nginx `docufit-port.conf` 로 **8080 HTTP 임시 공개**, 도메인 후 같은 nginx 에 80/443 + certbot. (5) 디스크가 빠듯해 release.sh 가 현재·이전 태그 외 이미지를 지운다.
- **Docker 변형 추가:** `Dockerfile`(node:22-bookworm-slim 3단계, prisma generate 용 더미 DATABASE_URL, `GIT_SHA` build-arg → RELEASE, 비루트 `node`, Node fetch 헬스체크), `.dockerignore`(.git·node_modules·.next·src/generated·docs·deploy·.env*·*.pem 제외), `deploy/docker/compose.yml`(read_only + tmpfs, cap_drop ALL, mem 768m, 로그 로테이션), `deploy/docker/setup-shared.sh`(전제 확인 → /srv/docufit/{compose.yml,app.env,.env,release.sh} → nginx 사이트(포트/도메인 모드) → 8080 ufw 임시 → certbot 옵션), `deploy/docker/release.sh`(pull → IMAGE_TAG 갱신 → up → 45초 헬스 → 롤백 → 이미지 정리), `deploy/nginx/docufit-port.conf`. `deploy-ec2.yml` 을 Docker 모드로 재작성(buildx + GHCR push, gha 캐시, scp compose/release.sh, `DEPLOY_URL` 변수로 헤더 확인). 전용 인스턴스 변형(`deploy/ec2/`)은 남기되 포트를 3100 으로 통일하고 "현재 워크플로 미사용" 명시. **로컬에 Docker 가 없어 이미지 빌드는 CI 첫 실행이 검증이다.**
- **다음:** 사용자: 서버에서 `git clone` → `sudo bash deploy/docker/setup-shared.sh` → 로컬 `docufit-ec2.pub` 등록(이미 `new-medi.pem` 으로 접속 가능하므로 선택) → 배포 키 `docufit-deploy` 생성·등록 → Secrets(EC2_HOST=3.34.61.39, EC2_USER=ubuntu, EC2_SSH_KEY) + Variables(EC2_DEPLOY_ENABLED=true, DEPLOY_URL=http://3.34.61.39:8080/) → 보안 그룹 8080 허용 → push/Run workflow → 첫 실행 뒤 GHCR 패키지 Public → `http://3.34.61.39:8080/` 확인 → 도메인 구매·A 레코드 → `setup-shared.sh --domain --email` + DEPLOY_URL 갱신. 그 뒤 GitHub Pages 워크플로 유지 여부, 개인정보 안내 3절(AWS 서울 EC2, nginx 접속 로그 보관 기간), 서버 재부팅(커널 업데이트, 다른 앱 운영자와 조율), SEO.
