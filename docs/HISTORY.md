...
- **빌드 상태:** `tsc` / `lint` / `build` 통과 (2026-09-10 세션 15). Chrome 헤드리스(CDP) 스모크: 스튜디오 30 + 에디터 30 + 병합/PDF 37 + 병합 중 편집 23 체크 통과. 실패 집계는 스크립트가 출력하는 `FAILS=` 값을 읽는다 (`grep -c '^FAIL'` 은 요약 줄까지 센다). HEIC 실파일 테스트는 미완 (사용자가 나중에 아이폰 사진으로 확인 예정)
